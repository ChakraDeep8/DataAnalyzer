...
lottie_coding = load_lottiefile("res/dataAnalysis_animate.json")
...
with col1:
    ...
    with st.expander("See what you can do 👇", expanded=True):
...
use_defo = st.checkbox('Use Sample Datasets')
if use_defo:
    select_dataset = st.selectbox('Select a dataset 👇',
                                  ['Car Price', 'Loan Eligibility', 'Diabetes', 'Hypertension', 'Stroke'])
    if "Car Price" in select_dataset:
        dataset = 'res/CarPrice.csv'
    if "Loan Eligibility" in select_dataset:
        dataset = 'res/Personal_loan.csv'
    if "Diabetes" in select_dataset:
        dataset = 'res/diabetes_data.csv'
    if "Hypertension" in select_dataset:
        dataset = 'res/hypertension_data.csv'
    if "Stroke" in select_dataset:
        dataset = 'res/stroke_data.csv'

st.sidebar.header('Import Dataset or use default datasets: 👉')
...
if dataset:
    ...
    if 'Count Plots of Categorical Columns' in vizuals:
        ...
        if len(cat_columns) == 0:
            ...
            selected_cat_cols = functions.sidebar_multiselect_container('Choose columns for Count plots:', cat_columns,
                                                                        'Count')
